...
BM25, semantic-tail, threshold, and low-information behavior without a model,
network, or database. Model quality is deliberately outside this gate; this
suite protects Chronicle's fusion and filtering contract while backends change.
"""
...
def _rank(case: dict, documents: list[dict], monkeypatch) -> list[str]:
    ...
    monkeypatch.setattr(
        search,
        "_rerank_scores",
        lambda _query, items: [float(rerank_scores.get(item["id"], 0.0)) for item in items],
    )
    return [item["id"] for item in search.search("eval-user", case["query"], limit=10)]
